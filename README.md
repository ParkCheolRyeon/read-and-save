...
Next.js 기반 PWA 텍스트 리더입니다. `public/books/`에 MDX 챕터 파일을 넣으면 로그인 이후 목록으로 보여주고, 세로 스크롤로 읽을 수 있습니다.

## 기능

- **정적 로그인**: ID/비밀번호 클라이언트 검증 (DB 없음)
- **책 목록**: `public/books/manifest.json`에 등록된 챕터 메타데이터 표시
- **리더**: 긴 텍스트 문서 스크롤 읽기, 로딩 스피너
...
1. 챕터 파일을 `public/books/`에 넣습니다. (예: `my-chapter.mdx`)
2. `public/books/manifest.json`을 열어 메타데이터 객체를 추가합니다.

   ```json
   [
     {
       "id": "my-chapter",
       "filename": "my-chapter.mdx",
       "title": "새 챕터"
     }
   ]
   ```
